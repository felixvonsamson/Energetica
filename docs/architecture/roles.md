...
| Manage instance whitelist | facilitator | instance | persistent | in-app | built (graduated from out-of-band; the roster lives in `accounts.db`, ADR-0006 — `scripts/whitelist-run.py` remains for a sysadmin who'd rather not go through the facilitator UI) |
