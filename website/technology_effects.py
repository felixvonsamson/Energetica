...
def price_multiplier(player, facility):
    ...
    if (
        ...
        mlt *= (
            const_config["assets"]["mechanical_engineering"]["price factor"]
            ** player.mechanical_engineering
        )
    # Physics
    if facility in const_config["assets"]["physics"]["affected facilities"]:
        mlt *= (
            const_config["assets"]["physics"]["price factor"] ** player.physics
        )
    # Mineral extraction
    if (
        facility
        in const_config["assets"]["mineral_extraction"]["affected facilities"]
    ):
        mlt *= (
    ...
    if (
        facility
        in const_config["assets"]["civil_engineering"]["affected facilities"]
    ):
    ...
    if (
        facility
        in const_config["assets"]["nuclear_engineering"]["affected facilities"]
    ):
        mlt *= (
            const_config["assets"]["nuclear_engineering"]["price factor"]
    ...
    if facility in engine.functional_facilities + engine.technologies:
        mlt *= const_config["assets"][facility]["price multiplier"] ** getattr(
            player, facility
        )
    ...
    if facility in engine.technologies:
        mlt *= (
            0.92
            ** engine.data["technology_lvls"][facility][
...
def power_multiplier(player, facility):
    ...
    if (
        ...
        mlt *= (
            const_config["assets"]["mechanical_engineering"]["prod factor"]
            ** player.mechanical_engineering
        )
    # Physics
    if facility in const_config["assets"]["physics"]["affected facilities"]:
        mlt *= (
            const_config["assets"]["physics"]["prod factor"] ** player.physics
        )
    ...
    if (
        ...
        mlt *= (
            const_config["assets"]["mineral_extraction"]["energy factor"]
            ** player.mineral_extraction
        )
    # Materials (in this case it is the energy consumption for construction)
    if facility in const_config["assets"]["materials"]["affected facilities"]:
        mlt *= (
            const_config["assets"]["materials"]["construction energy factor"]
            ** player.materials
        )
    # Civil engineering
    if (
...
def capacity_multiplier(player, facility):
    ...
    if facility in ["small_pumped_hydro", "large_pumped_hydro"]:
        mlt *= (
            const_config["assets"]["civil_engineering"]["capacity factor"]
            ** player.civil_engineering
        )
    return mlt
...
def efficiency_multiplier(player, facility):
    ...
    if (
        facility
        in const_config["assets"]["thermodynamics"]["affected facilities"]
    ):
        thermodynamic_factor = (
            const_config["assets"]["thermodynamics"]["efficiency_factor"]
            ** player.thermodynamics
        )
        if facility == "compressed_air":
            return (
                0.8
                / const_config["assets"][facility]["initial_efficiency"]
    ...
    if (
        ...
        mlt *= (
            const_config["assets"]["mineral_extraction"]["pollution factor"]
            ** player.mineral_extraction
        )
    # Chemistry
    if facility in const_config["assets"]["chemistry"]["affected facilities"]:
        chemistry_factor = (
            const_config["assets"]["chemistry"]["inefficiency_factor"]
            ** player.chemistry
        )
...
def construction_time_multiplier(player, facility):
    ...
    if facility in engine.functional_facilities + engine.technologies:
        mlt *= const_config["assets"][facility]["price multiplier"] ** (
            0.6 * getattr(player, facility)
        )
