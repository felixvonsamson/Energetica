...
function change_info(name) {
    ...
    let new_price =
        config[name]["price"] *
        config[name]["price multiplier"] **
            (ud[name]["lvl_future"] - ud[name]["lvl_at"]);
    ...
    if ("requirements" in config[name]) {
        ...
        for (let i in config[name]["requirements"]) {
            ...
            if (
                player_lvls[req] <
                config[name]["requirements"][i][1] + ud[name]["lvl_future"]
            ) {
    ...
    if (name == "thermodynamics") {
        ...
        let old_E_MS =
            ...
            (1 - config["molten_salt"]["efficiency"]) *
                0.9 ** (ud[name]["lvl_future"] - ud[name]["lvl_at"]);
