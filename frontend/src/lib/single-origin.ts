 ...
 * does not resolve, so the frontend must (a) build **same-origin** links
 * instead of cross-subdomain ones (see `apexDomain()` in `lib/instances.ts`)
 * and (b) collapse the multi-run chooser — there is exactly one reachable run,
 * so the picker bounces straight into it and the in-run switcher hides its
 * lateral hops.
 *
 ...
 * should see exactly which hosts flip it, and adding one is a one-line,
 * reviewed change. It ships in every bundle and is inert unless the current
 * host is listed.
 */
