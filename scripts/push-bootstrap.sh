...
# and reap-instances.sh (plus their Apache/systemd templates) live in scripts/infra/, but this
# is Option A (no git checkout on the server, ever — see docs/backend/deployment.md), so
# there is nothing for the normal deploy-instance.sh/deploy-lobby.sh rsync to target before
# the first instance or the lobby exists. These scripts must be delivered by hand.
#
