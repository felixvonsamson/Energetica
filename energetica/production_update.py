...
def set_facilities_usage(new_values: dict, player: Player) -> None:
    ...
    for controllable_facility in ControllableFacility:
...
def update_storage_lvls(new_values: dict, player: Player) -> None:
    ...
    for facility in StorageFacility:
...
def calculate_generation_without_market(new_values, player: Player) -> None:
    ...
    for facility in (*StorageFacility, *power_facility_types):
        if player.capacities.get(facility) is not None:
            internal_market = bid(
                internal_market,
                player.id,
    ...
    for facility in (*StorageFacility, *ControllableFacility):
        if player.capacities.get(facility) is not None:
...
def minimal_generation(player: Player, generation, resource_reservations: dict[Fuel, float]) -> None:
    ...
    for facility in (*ControllableFacility, *StorageFacility):
        if player.capacities.get(facility) is not None:
...
def resources_and_pollution(new_values, player: Player) -> None:
    ...
    for facility in ControllableFacility:
        if player.capacities.get(facility) is not None:
            for fuel, amount in player.capacities[facility]["fuel_use"].items():
                fuel = Fuel(fuel)
    ...
    # O&M costs
    for facility in (*power_facility_types, *StorageFacility, *ExtractionFacility):
        if player.capacities.get(facility) is not None:
            ...
            if isinstance(facility, ControllableFacility | ExtractionFacility):
                fc = 0.2
                if facility in ["nuclear_reactor", "nuclear_reactor_gen4"]:
                    fc = 0.5
                if isinstance(facility, ExtractionFacility):
...
def construction_emissions(new_values, player: Player) -> None:
    ...
    for ud in OngoingProject.filter_by(player=player):
        if ud.is_ongoing() and isinstance(ud, Technology):
...
def reduce_demand(new_values, demand_type, player_id, satisfaction) -> None:
    ...
    if isinstance(demand_type, ExtractionFacility | StorageFacility) or demand_type == "carbon_capture":
        return
