...
def renewables_generation(player: Player, generation: dict) -> None:
    ...
    # HYDRO
    power_factor = hydro_power_fraction(calculate_river_speed(in_game_seconds_passed, engine.days_per_year))
    ...
    for facility_type in SolarFacilityType:
        if player.capacities.get(facility_type) is not None:
            for facility in ActiveFacility.filter_by(player=player, facility_type=facility_type):
                ...
                facility.usage = solar_power_fraction(irradiance)
        ...
        if player.capacities.get(facility_type) is not None:
            for facility in ActiveFacility.filter_by(player=player, facility_type=facility_type):
                wind_speed = calculate_wind_speed(
                    facility.position, in_game_seconds_passed, engine.random_seed, engine.days_per_year
                )
                max_power = (
                    engine.const_config["assets"][facility_type]["base_power_generation"]
                    * facility.multipliers["power_production_multiplier"]
                ...
                facility.usage = wind_power_fraction(effective_wind_speed)
                generation[facility_type] += facility.usage * max_power
                facility.cut_out_speed_exceeded = bool(effective_wind_speed >= WIND_CUT_OUT_SPEED)
